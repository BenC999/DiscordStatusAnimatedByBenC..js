//META{"name":"AnimatedStatus","source":"https://raw.githubusercontent.com/BenC999/DiscordStatusAnimated/main/AnimatedDiscordStatus.js?token=ASW4ORFVXOTDC4QXQFM4U7TADLLUS","website":"https://github.com/BenC999/DiscordStatusAnimated/blob/main/AnimatedDiscordStatus.js"}*//
...
class AnimatedStatus {
	...
	getDescription () {
		return "גורם לסטטוס הדיסקורד שלך לזוז.";
	}
	...
	newRichRow (text, emoji, optNitroId = undefined) {
		...
		optNitroIdWidget.placeholder = "גיף - ID";
	...
	getSettingsPanel () {
		...
		// Auth token
		settings.appendChild(GUI.newLabel("שים פה את טוקן המשתמש שלך - (https://discordhelp.net/discord-token)"));
		...
		// timeout
		settings.appendChild(GUI.newLabel("בחר זמן תזוזה (מאיות)"));
		...
		// Animation
		settings.appendChild(GUI.newLabel('בחר אילו דברים תרצה להשים בסטטוס שלך.'));
		...
		let addStep = GUI.newButton("הוסף", false);
		...
		let delStep = GUI.newButton("הורד", false);
		...
		let changeEditMode = GUI.newButton("שנה אופציית עריכה");
		...
		save.onclick = () => {
			...
			// Show Toast
			BdApi.showToast("שמרת את ההגדרות.", {type: "success"});
